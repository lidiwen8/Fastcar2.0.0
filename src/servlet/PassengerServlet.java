...
public class PassengerServlet extends HttpServlet {
    ...
    private void queryinput(HttpServletRequest request, HttpServletResponse response) throws Exception {
        response.setContentType("text/html;charset=UTF-8");
        ...
        String requestUrl = "http://api.map.baidu.com/place/v2/suggestion?region=REGION&query=QUERY&city_limit=true&output=json&ak=94Tmshjhp03oul7xy95Gu3wwHkjGZvkk&mcode=EE:0C:C8:50:54:53:96:5A:55:8C:23:2F:93:7E:EB:AE:D8:C8:1B:F1;com.example.tangdekun.androidannotationsdemo";
        requestUrl = requestUrl.replace("QUERY", query).replace("REGION", region);
        JSONObject jsonObject = util.CommonUtil.httpsRequest(requestUrl, "POST", "");
...
//							return;
    ...
    private void createOrder(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        String origin;
        String destination;
        if (request.getParameter("theid1") == null) {
            origin = request.getParameter("origin");
        } else {
            origin = request.getParameter("theid1");
        }
        if (request.getParameter("theid2") == null) {
            destination = request.getParameter("destination");
        } else {
            destination = request.getParameter("theid2");
        }
        ...
        String[] o1 = LocationUtil.getCoordinate(city1 + origin);
        String[] o2 = LocationUtil.getCoordinate(city2 + destination);
        BigDecimal bd1 = new BigDecimal(o1[0]);
        BigDecimal bd2 = new BigDecimal(o1[1]);
        BigDecimal bd3 = new BigDecimal(o2[0]);
        BigDecimal bd4 = new BigDecimal(o2[1]);
        HttpSession session = request.getSession();
        ...
        order.setDistance(LocationUtil.getDistance(bd1.doubleValue(),bd2.doubleValue(),bd3.doubleValue(),bd4.doubleValue()));
