...
<nav class="navbar navbar-inverse">
    <p class="navbar-text">平台乘客信息</p>
</nav>
<div class="container">
    <h2 class="sub-header">乘客信息分页列表</h2><br>
    <div class="table-responsive">
        <table class="table table-striped">
            ...
            <c:forEach items="${pb.beanList}" var="cstm">
                <tbody>
                <tr>
                    <td>${cstm.passengerid}</td>
                    <td>${cstm.name}</td>
                    <td>${cstm.number}</td>
                    <td>${cstm.sex}</td>
                    <td>${cstm.logintime}</td>
                    <td>${cstm.registertime}</td>
                    <%--<td>--%>
                        <%--<button class="btn btn-danger btn-sm delete_btn" onclick="javascript:window.location.href='admin/index.jsp'">返回</button>--%>
                    <%--</td>--%>
                </tr>
                </tbody>
            </c:forEach>
